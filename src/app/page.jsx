...
export default function HomePage () {
  const { tasks, handleDeleteTask, handleChangeStatus, handleCreateTask, handleUpdateTask, errors } = useHomePage()
  return (
      ...
      <div className={`grid grid-rows-none gap-4 place-items-center ${inter.className} w-full`}>
        {tasks?.map((task) =>
          <Suspense fallback={<LoadingTask />} key={task.id}>
            <LazyTask taskData={task} key={task.id} actionOnUpdate={handleUpdateTask} actionOnDelete={handleDeleteTask} actionOnComplete={handleChangeStatus} />
          </Suspense>
        )}
      </div>
    </div>
