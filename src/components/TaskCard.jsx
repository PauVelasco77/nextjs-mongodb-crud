    ...
    <div className=' bg-gray-500 p-2 rounded-lg transition-colors hover:border-secondary2 flex justify-between items-center gap-5 break-keep w-full border-2 border-gray-400'>
      <div className='flex items-center gap-4'>
        ...
        <div className='flex gap-1'>
          <input type='text' onChange={handleUpdate} name={TASK.title} value={task.title} className={`text-sm whitespace-nowrap font-normal bg-transparent outline-none ${task.complete ? 'line-through text-gray-300' : 'text-gray-100'}`} />
        </div>
      </div>
